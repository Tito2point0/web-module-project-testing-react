...
test("renders the summary test passed as prop", () => {
    render(<Episode episode={exampleEpisodeData} />);
    const summary = screen.queryByText(/A young/i);
    console.log(summary)
...
test("renders default image when image is not defined", () => {
    render(<Episode episode={noImgExampleEpisodeData} />);
    const imgSummary = screen.queryByAltText('https://i.ibb.co/2FsfXqM/stranger-things.png')
    console.log(imgSummary)
    expect(imgSummary).toBeTruthy();
    expect(imgSummary).toBeInTheDocument();
